...
export default function HotelMediaPage() {
  ...
  // Definición de categorías con colores
  const categories: MediaCategory[] = [
    { id: "all", label: "Todas", color: "bg-blue-600" },
    { id: "exterior", label: "Exterior", color: "bg-green-500" },
    { id: "rooms", label: "Habitaciones", color: "bg-purple-500" },
    { id: "interior", label: "Interior", color: "bg-amber-500" },
    { id: "common_areas", label: "Salas comunes", color: "bg-indigo-500" },
    { id: "restaurant", label: "Restaurante", color: "bg-rose-500" },
    { id: "pool", label: "Piscina", color: "bg-cyan-500" },
    { id: "spa", label: "Spa", color: "bg-pink-500" },
    { id: "other", label: "Otros", color: "bg-gray-500" }
  ];
  ...
  useEffect(() => {
    const loadOrg = async () => {
      try {
        // ✅ CORRECCIÓN: Usar getUser() correctamente para el cliente
        ...
        if (orgData.role === "hotel") {
          // Incluir la columna 'versions' en la consulta
          const { data: mediaData } = await supabase
            .from("media")
            .select("id, url, type, versions")
            .eq("hotel_id", orgData.id)
            .order("created_at", { ascending: false });

          // Parsear las versiones de JSON string a objeto
  ...
  // Filtrar media por categoría
  const filteredMedia = activeCategory === "all" 
    ? media 
    : media.filter(item => item.type === activeCategory);
        ...
        <div className="pt-24">
          ...
          {org.role === "hotel" && (
            <div className="max-w-7xl mx-auto bg-white rounded-2xl shadow-lg overflow-hidden">
              <div className="p-8">
                {/* Barra de categorías exactamente como en la imagen */}
                ...
                {/* PASAR filteredMedia AL GALLERY */}
                <Gallery key={filteredMedia.length} media={filteredMedia} />
              </div>
            </div>
          )}
